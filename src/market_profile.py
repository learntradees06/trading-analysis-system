...
class MarketProfile:
    ...
    def classify_opening_type(self, current_open: float, prior_profile: Dict) -> str:
        """
        Classify the opening type based on the current open relative to the
        previous day's range (High/Low) and Close.

        Args:
            current_open: Today's opening price.
            prior_profile: Dictionary containing the previous day's profile data,
                           including 'session_high', 'session_low', and 'session_close'.

        Returns:
            Opening type classification: 'HOR', 'HIR', 'LIR', or 'LOR'.
        """
        if not prior_profile or not all(k in prior_profile for k in ['session_high', 'session_low', 'session_close']):
            return 'Unknown'

        prior_high = prior_profile['session_high']
        prior_low = prior_profile['session_low']
        prior_close = prior_profile['session_close']

        if current_open > prior_high:
            return 'HOR'  # Higher Outside Range
        elif prior_close <= current_open <= prior_high:
            return 'HIR'  # Higher Inside Range
        elif prior_low <= current_open < prior_close:
            return 'LIR'  # Lower Inside Range
        elif current_open < prior_low:
            return 'LOR'  # Lower Outside Range
        else:
            return 'Unknown'
