...
class TradingSystem:
    ...
    def generate_trading_plan(self, is_next_day: bool = True):
        ...
        try:
            ...
            hourly_data = self.data_manager.fetch_data('4h', days_back=30)  # Changed from 1h to 4h
            ...
            # Check if all models exist, if not, run full training
            all_models_exist = all(self.ml_predictor.model_exists(target) for target in ['target_broke_ibh', 'target_broke_ibl', 'target_next_day_direction'])

            if not all_models_exist:
                print("⚠️ One or more ML models not found. Running full training process...")
                self.train_ml_models() # This will train and save the models
                # We need to reload the models after training
                for target in ['target_broke_ibh', 'target_broke_ibl', 'target_next_day_direction']:
                    self.ml_predictor.load_model(target)
